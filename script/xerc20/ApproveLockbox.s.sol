...
contract ApproveLockbox is Script {
    ...
    AetXERC20Lockbox old = AetXERC20Lockbox(router);
    ...
    function run() external {
        ...
        AetXERC20Lockbox logic = new AetXERC20Lockbox(lockbox, 1, mailbox);
        proxyAdmin.upgradeAndCall(
            proxy,
            address(logic),
            abi.encodeCall(AetXERC20Lockbox.approveLockbox, ())
        );
        vm.stopBroadcast();

        vm.expectRevert("Initializable: contract is already initialized");
        AetXERC20Lockbox(address(proxy)).initialize(
            address(0),
            address(0),
            mailbox
        );

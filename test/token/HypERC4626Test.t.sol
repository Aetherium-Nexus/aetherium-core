...
import {AetTokenTest} from "./AetERC20.t.sol";
import {MockMailbox} from "../../contracts/mock/MockMailbox.sol";
import {AetERC20} from "../../contracts/token/AetERC20.sol";
import {AetERC4626Collateral} from "../../contracts/token/extensions/AetERC4626Collateral.sol";
...
contract AetERC4626CollateralTest is AetTokenTest {
    ...
    AetERC4626Collateral internal rebasingCollateral;
    MockERC4626YieldSharing vault;

    MockMailbox internal peerMailbox; // mailbox for second synthetic token
    AetERC20 internal peerToken;

    AetERC4626Collateral localRebasingToken;
    AetERC4626 remoteRebasingToken;
    AetERC4626 peerRebasingToken;
    ...
    function setUp() public override {
        ...
        AetERC4626Collateral implementation = new AetERC4626Collateral(
            vault,
            SCALE,
        ...
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            ...
            abi.encodeWithSelector(
                AetERC4626Collateral.initialize.selector,
                address(address(noopHook)),
                address(0x0),
                address(this)
            )
        );
        localToken = AetERC4626Collateral(address(proxy));

        remoteToken = new AetERC4626(
            primaryToken.decimals(),
            SCALE,
            address(remoteMailbox),
            localToken.localDomain()
        );
        peerToken = new AetERC4626(
            primaryToken.decimals(),
            SCALE,
            address(peerMailbox),
            localToken.localDomain()
        ...
        localRebasingToken = AetERC4626Collateral(address(proxy));
        remoteRebasingToken = AetERC4626(address(remoteToken));
        peerRebasingToken = AetERC4626(address(peerToken));

...
import {AetERC20} from "../../contracts/token/AetERC20.sol";
...
import {AetFiatToken} from "../../contracts/token/extensions/AetFiatToken.sol";
import {AetNative} from "../../contracts/token/AetNative.sol";
import {TokenRouter} from "../../contracts/token/libs/TokenRouter.sol";
import {TokenMessage} from "../../contracts/token/libs/TokenMessage.sol";
import {Message} from "../../contracts/libs/Message.sol";

abstract contract AetTokenTest is Test {
    ...
    string internal constant NAME = "AetheriumInu";
    string internal constant SYMBOL = "AET";
    ...
    function setUp() public virtual {
        ...
        AetERC20 implementation = new AetERC20(
            DECIMALS,
            SCALE,
        ...
        remoteToken = AetERC20(address(proxy));
...
contract AetERC20Test is AetTokenTest {
    using TypeCasts for address;

    AetERC20 internal erc20Token;
    ...
    function setUp() public override {
        ...
        localToken = AetERC20(address(proxy));
        erc20Token = AetERC20(address(proxy));
...
contract AetERC20CollateralTest is AetTokenTest {
    using TypeCasts for address;

    AetERC20Collateral internal erc20Collateral;

    function setUp() public override {
        super.setUp();

        localToken = new AetERC20Collateral(
            address(primaryToken),
            SCALE,
            address(localMailbox)
        );
        erc20Collateral = AetERC20Collateral(address(localToken));
    ...
    function test_constructor_revert_ifInvalidToken() public {
        vm.expectRevert("AetERC20Collateral: invalid token");
        new AetERC20Collateral(address(0), SCALE, address(localMailbox));
    }
...
contract AetXERC20Test is AetTokenTest {
    using TypeCasts for address;

    AetXERC20 internal xerc20Collateral;
    ...
    function setUp() public override {
        ...
        localToken = new AetXERC20(
            address(primaryToken),
            SCALE,
            address(localMailbox)
        );
        xerc20Collateral = AetXERC20(address(localToken));
...
contract AetXERC20LockboxTest is AetTokenTest {
    using TypeCasts for address;

    AetXERC20Lockbox internal xerc20Lockbox;
    ...
    function setUp() public override {
        ...
        localToken = new AetXERC20Lockbox(
            address(lockbox),
            SCALE,
            address(localMailbox)
        );
        xerc20Lockbox = AetXERC20Lockbox(address(localToken));
...
contract AetFiatTokenTest is AetTokenTest {
    using TypeCasts for address;

    AetFiatToken internal fiatToken;
    ...
    function setUp() public override {
        ...
        localToken = new AetFiatToken(
            address(primaryToken),
            SCALE,
            address(localMailbox)
        );
        fiatToken = AetFiatToken(address(localToken));
...
contract AetNativeTest is AetTokenTest {
    using TypeCasts for address;

    AetNative internal nativeToken;

    function setUp() public override {
        super.setUp();

        localToken = new AetNative(SCALE, address(localMailbox));
        nativeToken = AetNative(payable(address(localToken)));
...
contract AetERC20ScaledTest is AetTokenTest {
    using TypeCasts for address;

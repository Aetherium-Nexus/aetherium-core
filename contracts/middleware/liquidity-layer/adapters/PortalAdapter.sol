...
contract PortalAdapter is ILiquidityLayerAdapter, Router {
    ...
    /// @notice Aetherium domain => Wormhole domain.
    mapping(uint32 => uint16) public aetheriumDomainToWormholeDomain;
     ...
     * @param destination The aetherium domain of the destination
     */
    event BridgedToken(
        uint256 nonce,
    ...
    /**
     * @notice Emitted when the Aetherium domain to Wormhole domain mapping is updated.
     * @param aetheriumDomain The Aetherium domain.
     * @param wormholeDomain The Wormhole domain.
     */
    event DomainAdded(uint32 indexed aetheriumDomain, uint32 wormholeDomain);
     ...
     * @param _destinationDomain The aetherium domain of the destination
    ...
    function sendTokens(
        ...
        nonce = nonce + 1;
        uint16 _wormholeDomain = aetheriumDomainToWormholeDomain[
            _destinationDomain
        ];
     ...
     * @param _originDomain The aetherium domain of the origin
     * @param _recipient The address of the recipient
     * @param _amount The amount of tokens to send
     * @param _adapterData The adapter data from the origin chain, containing the nonce
     */
    function receiveTokens(
        uint32 _originDomain, // Aetherium domain
        address _recipient,
        uint256 _amount,
    ...
    function completeTransfer(bytes memory encodedVm) public {
        ...
        address tokenAddress = _transfer.tokenChain ==
            aetheriumDomainToWormholeDomain[localDomain]
    ...
    // and doesn't actually send/handle Aetherium messages directly
    function _handle(
        uint32, // origin
        bytes32, // sender
    ...
    function addDomain(
        uint32 _aetheriumDomain,
        uint16 _wormholeDomain
    ) external onlyOwner {
        aetheriumDomainToWormholeDomain[_aetheriumDomain] = _wormholeDomain;

        emit DomainAdded(_aetheriumDomain, _wormholeDomain);
    }

    /**
     * The key that is used to track fulfilled Portal transfers
     * @param _aetheriumDomain The aetherium of the origin
    ...
    function transferId(
        ...
        return bytes32(abi.encodePacked(_aetheriumDomain, _nonce));
    }
}

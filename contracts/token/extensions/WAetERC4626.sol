...
import {AetERC4626} from "./AetERC4626.sol";
 ...
 * @title WAetERC4626
 * @author Abacus Works
 * @notice A wrapper for AetERC4626 that allows for wrapping and unwrapping of underlying rebasing tokens
 */
contract WAetERC4626 is ERC20, PackageVersioned {
    AetERC4626 public immutable underlying;

    constructor(
        AetERC4626 _underlying,
        string memory name,
    ...
    function wrap(uint256 _underlyingAmount) external returns (uint256) {
        require(
            _underlyingAmount > 0,
            "WAetERC4626: wrap amount must be greater than 0"
        );
    ...
    function unwrap(uint256 _wrappedAmount) external returns (uint256) {
        require(
            _wrappedAmount > 0,
            "WAetERC4626: unwrap amount must be greater than 0"
        );
